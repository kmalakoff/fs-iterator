...
PathStack.prototype.destroy = function destroy() {
  this.iterator = null;
  this.stack = null;
  this.length = 0;
};
...
PathStack.prototype.push = function push(item) {
  ...
  if (!this.iterator.done) drainStack(this.iterator);
  return this;
};
...
PathStack.prototype.pop = function pop() {
  ...
  // resuse this item
  if (item.files.length > 1) return { path: item.path, basename: item.files.pop(), depth: item.depth };
...
PathStack.prototype.peek = function peek() {
  return this.stack.length ? this.stack.last() : undefined;
};
