...
module.exports = function path(item, options, callback) {
  ...
  var first = options.alwaysStat ? stat : filter;
  var second = options.alwaysStat ? filter : stat;

  // choose an order
  every([first, second], entry, item, options, function filterAndStatsCallback(err, keep) {
