...
module.exports = function path(item, options, callback) {
  ...
  every(
    options.alwaysStat
      ? [stats.bind(null, entry, item, options), filter.bind(null, entry, options)]
      : [filter.bind(null, entry, options), stats.bind(null, entry, item, options)],
    function filterAndStatsCallback(err, keep) {
