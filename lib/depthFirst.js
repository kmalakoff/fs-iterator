...
function processPath(iterator, paths, depth, callback) {
  ...
  iterator.options.stat(entry.fullPath, function (err1, stats) {
    ...
    if (stats.isDirectory()) processDirectory(iterator, paths, depth, entry, stats, callback);
    else processFile(iterator, paths, entry, stats, callback);
  });
}

function processFile(iterator, paths, entry, stats, callback) {
  processFilter(iterator, entry.path, stats, function (err, keep) {
...
function processDirectory(iterator, paths, depth, entry, stats, callback) {
